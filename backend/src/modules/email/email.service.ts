...
export const sendVerificationEmail = async (to: string, token: string, name: string): Promise<void> => {
  ...
  const verificationEmailUrl = `${config.clientUrl}/verify-email`;
  const text = `Hi ${name},
  Here is your OTP to verify email on OpsInsight: 
  <h3>${token}</h3>
 ...
 * @returns {Promise<void>}
 */
export const sendSuccessfulRegistration = async (to: string, { verifyEmailToken, verifyOTP } : { verifyEmailToken: string, verifyOTP: string }, name: string): Promise<void> => {
  const subject = 'Email Verification';
  // replace this url with the link to the email verification page of your front-end app
  const verificationEmailUrl = `${config.clientUrl}/verify-email?token=${verifyEmailToken}`;
  const text = `Hi ${name},
  Congratulations! Your account has been created. 
  You are almost there. Complete the final step by verifying this email on OpsInsight using the below OTP: ${verifyOTP}
  Verifying your email at: ${verificationEmailUrl}
  Don't hesitate to contact us if you face any problems
  Regards,
  Team`;
  const html = `<div style="margin:30px; padding:30px; border:1px solid black; border-radius: 20px 10px;"><h4><strong>Hi ${name},</strong></h4>
  <p>Congratulations! Your account has been created.</p>
  <p>You are almost there. Complete the final step by verifying this email on OpsInsight using the below OTP:</p>
  <h3>${verifyOTP}</h3>
  <p>Verifying your email at: ${verificationEmailUrl}</p>
